...
>数据存储：所有客户数据存储在Microsoft Azure服务中。 用户需要经 Azure AD 验证的 2 因素。 基于角色 (RBAC) 已就位。 对加密Microsoft Azure完全通过加密连接进行。 所有数据都进行其余加密。 所有服务都受 Azure 安全中心最佳保护。 
