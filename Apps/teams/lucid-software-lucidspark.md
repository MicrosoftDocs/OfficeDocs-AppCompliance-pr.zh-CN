...
此信息由 Lucid Software 提供，与此应用程序如何收集和存储组织数据以及组织将拥有对应用收集的数据的控制有关。
...
>|:----------------|:--------------------|:---------------------------------------------------|:--------------------------|:--------------------------|
>| email | delegated | 姓名和电子邮件地址。 | 电子邮件、openid 和配置文件权限使 Lucidspark 可以生成用户的 openid 令牌，并获取关于该用户的足够基本信息，以便在必要时为用户注册 Lucidspark 帐户。 为了验证从 Microsoft 返回的数据，我们请求获取其响应已签名的公钥。 作为 SSO 流的一部分，不会从 Microsoft 接收或发送给 Microsoft 其他数据。 | 3557d5c0-bcab-410b-8a03-f7045aa48de0 |
>| openid | delegated | 姓名和电子邮件地址。 | 电子邮件、openid 和配置文件权限使 Lucidspark 可以生成用户的 openid 令牌，并获取关于该用户的足够基本信息，以便在必要时为用户注册 Lucidspark 帐户。 为了验证从 Microsoft 返回的数据，我们请求获取其响应已签名的公钥。 作为 SSO 流的一部分，不会从 Microsoft 接收或发送给 Microsoft 其他数据。 | 3557d5c0-bcab-410b-8a03-f7045aa48de0 |
>| 个人资料 | delegated | 姓名和电子邮件地址。 | 电子邮件、openid 和配置文件权限使 Lucidspark 可以生成用户的 openid 令牌，并获取关于该用户的足够基本信息，以便在必要时为用户注册 Lucidspark 帐户。 为了验证从 Microsoft 返回的数据，我们请求获取其响应已签名的公钥。 作为 SSO 流的一部分，不会从 Microsoft 接收或发送给 Microsoft 其他数据。 | 3557d5c0-bcab-410b-8a03-f7045aa48de0 |
