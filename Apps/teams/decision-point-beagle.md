...
| 应用的核心功能 | Beagle 是基于聊天的 BI 工具，与 MS Teams 等平台集成。 Beagle 通过协作应用向用户 (数据，) 用户的培训，从而降低采用障碍，并消除对个人培训需求。  Beagle 以见解卡片的形式向用户显示相关的数据，并允许他们使用筛选器深入探究这些数据，并帮助加快决策制定。  |
| 公司总部位置 | 印度 |
| 应用信息页面 | [https://beagleanalytics.ai](https://beagleanalytics.ai) |
| 用于运行应用的托管环境或服务模型是什么？ |  |
