...
>否。 在 SO Teams 产品Teams MS 和 SO Teams映射。 MS Teams向我们发送其可识别 ID，我们会将其保存在内部以映射用户。 此外，MS Teams会向我们发送聊天机器人请求的 JWT (防止请求伪造) 和 Tab 请求使用 SO Cookie 进行验证。
