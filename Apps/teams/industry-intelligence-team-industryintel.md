...
>不正确。 在 SO Teams 产品Teams MS 和 SO Teams映射。 MS Teams向我们发送其可识别 ID，我们将其保存在内部以映射用户。 此外，MS Teams会向我们发送自动程序请求的 JWT (阻止请求伪造) 使用 SO Cookie 验证 Tab 请求。
