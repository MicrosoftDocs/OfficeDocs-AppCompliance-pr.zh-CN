...
1. Azure Active Directory API (对象 ID aad_object_id) 存储为从用户Microsoft Teams获取&#8217;ID。 使用aad_object_id上的 Microsoft&#8217;聊天机器人 sdk，可以安全地检索到此内容。

2. 在 prezi.com 上创建的视频 prezi.com。 在 prezi.com 上创建的内容按以下 URL 的第 14 节存储： https://prezi.com/privacy-policy/ 

对高风险外部系统的访问权限 (AWS) 通过 OneLogin (的第三方统一标识和访问管理) 。
