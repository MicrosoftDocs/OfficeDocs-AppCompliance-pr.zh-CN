...
| 应用的核心功能 | XBRL US 是一家非盈利性协会，它在美国 XBRL Exchange US 中构建和支持免费、开放、非商业报告标准，引导了针对政府机构（美国证券交易委员会 (SEC) 、联邦能源监管委员会 (FERC) 和能源部）制定和实施数据标准。 XBRL US 监视数据并帮助构建 SEC 使用的数据质量标准，以帮助公共公司创建高质量的 XBRL 财务报表。  |
| 公司总部位置 | 美国 |
| 应用信息页面 | |
| 用于运行应用的托管环境或服务模型是什么？ |  |
