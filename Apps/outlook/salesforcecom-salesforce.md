...
| 应用的核心功能 | 让你能够体验 Salesforce Outlook。 集成是 100% 云，无需任何软件推送、安装或维护。 在不在 Salesforce 和 Outlook 之间切换的情况下查看与电子邮件和约会关联的所有联系人、潜在客户、帐户、机会和事例;Salesforce 数据直接显示在 Outlook。 直接在 Salesforce 中创建新的 Salesforce 记录，包括Outlook。 只需单击一下即可将电子邮件和约会添加到 Salesforce 记录，即使在撰写时也一样 |
| 公司总部位置 | 美国 |
| 应用信息页面 | |
| 用于运行应用的托管环境或服务模型是什么？ |  |
