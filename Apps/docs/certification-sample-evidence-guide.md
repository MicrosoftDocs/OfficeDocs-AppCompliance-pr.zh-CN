...
<a id="control-1"></a>[控制 #1](#control-1)：提供管理防病毒做法和过程的策略文档。
...
<a id="control-2"></a>[控制 #2](#control-2)：提供证明反病毒软件正在所有采样的系统组件中运行的明显证据。
...
<a id="control-3"></a>[控制 #3](#control-3)：提供可证明的证据，证明防病毒签名在) 1天内 (的所有环境中都是最新的。
...
<a id="control-4"></a>[控制 #4](#control-4)：提供可证明的证据，证明防病毒已配置为对所有采样的系统组件执行访问扫描或定期扫描。
...
<a id="control-5"></a>[控制 #5](#control-5)：提供可证明的证据，证明防病毒配置为自动阻止所有采样的系统组件中的恶意软件或隔离和警报。
...
<a id="control-6"></a>[控制 #6](#control-6)：提供可证明的证据，证明应用程序在部署之前已获得批准。

- 意向：通过应用程序控制，组织将批准允许在操作系统上运行的每个应用程序/进程。 此控件的目的是确保已完成审批过程，以授权哪些应用程序/进程可以运行。

- 示例证据准则：可以提供证据，表明正在遵循审批过程。 这可以通过已签名的文档、在更改控制系统中跟踪或使用类似Azure DevOps或JIRA来跟踪这些请求和授权来提供。
...
<a id="control-7"></a>[控制 #7](#control-7)：提供可证明的证据，证明存在并维护了具有业务理由的已批准应用程序的完整列表。
...
<a id="control-8"></a>[控制 #8](#control-8)：提供详细介绍应用程序控制软件配置为满足特定应用程序控制机制的支持文档。
...
<a id="control-9"></a>[控件 #9](#control-9)：提供可证明的证据，证明应用程序控件是根据所有采样系统组件中所述进行配置的。
...
<a id="control-10"></a>[控制 #10](#control-10)：提供策略文档，用于控制如何识别和分配新的安全漏洞并分配风险分数。
...
<a id="control-11"></a>[控制 #11](#control-11)：提供如何识别新安全漏洞的证据。

- 意向：此控件的目的是确保遵循此过程，并且它&#39;足够强大，能够识别整个环境中的新安全漏洞。 这可能不只是操作系统;它可能包括在环境中运行的应用程序和任何代码依赖项。

- 示例证据指南：可以通过显示邮寄列表订阅的方式提供证据，手动查看安全源以了解新发布的漏洞， (需要使用活动的时间戳（例如，使用JIRA或Azure DevOps) ）进行充分跟踪，例如，查找过期软件 (的工具在查找过期软件库时可能是 Snyk， 或者可以是 Nessus，使用经过身份验证的扫描来识别过期的 software.) 。
...
<a id="control-12"></a>[控制 #12](#control-12)：提供证据，证明所有漏洞在标识后都分配了风险排名。
...
<a id="control-13"></a>[控制 #13](#control-13)：提供用于修补范围内系统组件的策略文档，其中包括针对关键、高和中等风险漏洞的适当最小修补时间范围;并解除任何不受支持的操作系统和软件的授权。
...
<a id="control-14"></a>[控制 #14](#control-14)：提供可证明的证据，证明所有采样的系统组件正在修补。
...
<a id="control-15"></a>[控制 #15](#control-15)：提供可证明的证据，证明环境中未使用任何不受支持的操作系统和软件组件。
...
<a id="control-16"></a>[控制 #16](#control-16)：提供季度基础结构和 Web 应用程序漏洞扫描报告。 需要针对整个公共占用情况进行扫描， (IP 地址和 URL) 和内部 IP 范围。
...
<a id="control-17"></a>[控制 #17](#control-17)：提供可证明的证据，证明在漏洞扫描期间识别的漏洞修正已根据记录的修补时间范围进行修补。
...
<a id="control-18"></a>[控制 #18](#control-18)：提供管理防火墙管理做法和过程的策略文档。
...
<a id="control-19"></a>[控制 #19](#control-19)：提供明显的证据表明，在安装到生产环境中之前，任何默认管理凭据都已更改。
...
<a id="control-20"></a>[控制 20](#control-20)：提供可证明的证据，证明防火墙安装在范围内环境的边界上，并在外围网络 (（也称为 DMZ、非军事化区域和受筛选的子网）之间安装，) 和内部受信任的网络。
...
<a id="control-21"></a>[控制 21](#control-21)：提供明显的证据，证明所有公共访问在非军事区 (DMZ) 中终止。
...
<a id="control-21"></a>[控制 22](#control-22)：提供明显的证据，证明通过防火墙允许的所有流量都经过审批过程。
...
- 示例证据准则：证据可以是文档形式，其中显示了已授权的防火墙更改请求，这可能是 CAB (更改顾问委员会) 会议或更改控制系统跟踪所有更改的分钟数。
...
<a id="control-23"></a>[控制 23](#control-23)：提供明显的证据，证明防火墙规则基础配置为删除未显式定义的流量。
...
<a id="control-24"></a>[控制 24](#control-24)：提供明显的证据，证明防火墙仅支持所有非控制台管理接口上的强加密。
...
<a id="control-25"></a>[控制 25](#control-25)：提供可证明的证据，证明你至少每 6 个月执行一次防火墙规则评审。
...
<a id="control-26"></a>[控制 26](#control-26)：提供可证明的证据，证明Web 应用程序防火墙 (WAF) 配置为主动监视、警报和阻止恶意流量。
...
<a id="control-27"></a>[控制 27](#control-27)：提供可证明 WAF 支持 SSL 卸载的证据。
...
<a id="control-28"></a>[控制 28](#control-28)：“根据 OWASP 核心规则集 (3.0 或 3.1) 提供可证明的证据，证明 WAF 可以防范某些或所有以下类别的漏洞：
- 协议和编码问题，
- 标头注入、请求走私和响应拆分，
- 文件和路径遍历攻击，
- 远程文件包含 (RFI) 攻击，
...
<a id="control-29"></a>[控制 29](#control-29)：提供管理更改控制过程的策略文档。
...
<a id="control-30"></a>[控制 30](#control-30)：提供可证明的证据，证明开发和测试环境强制将职责与生产环境分离。
...
<a id="control-31"></a>[控制 31](#control-31)：提供可证明的证据，证明在开发或测试环境中未使用敏感生产数据。
...
<a id="control-32"></a>[控制 32](#control-32)：提供可证明的证据，证明记录的更改请求包含更改的影响、回退过程的详细信息以及要执行的测试。
...
<a id="control-33"></a>[控制 33](#control-33)：提供可证明的证据，证明更改请求经过授权和注销过程。
...
<a id="control-34"></a>[控制 34](#control-34)：提供支持安全软件开发和部署的策略和过程，包括针对常见漏洞类（例如 OWASP 前 10 名或 SANS 前 25 CWE）的安全编码最佳做法指南。
...
<a id="control-35"></a>[控制 35](#control-35)：提供可证明的证据，证明代码更改由第二个审阅者进行审阅和授权过程。
...
<a id="control-36"></a>[控制 36](#control-36)：提供可证明的证据，证明开发人员每年接受安全软件开发培训。
...
<a id="control-37"></a>[控制 37](#control-37)：提供可证明的证据，证明代码存储库通过多重身份验证 (MFA) 进行保护。
...
<a id="control-38"></a>[控制 38](#control-38)：提供可证明的证据，证明访问控制已到位，可以保护代码存储库。
...
<a id="control-39"></a>[控制 39](#control-39)：提供管理帐户管理做法和过程的策略文档。

- 意向：用户帐户继续成为威胁执行组件的目标，并且通常是数据泄露的源。 通过配置过于宽松的帐户，组织不仅会增加&#39;特权&#39;帐户的池，这些帐户可由威胁执行组件利用来执行数据泄露，而且还会增加成功利用需要特定特权才能成功的漏洞的风险。

-  BeyondTrust 每年都会生成一份 Microsoft 漏洞报告&quot;，用于分析上一&quot;年的 Microsoft 安全漏洞，并详细了解依赖拥有管理员权限的用户帐户的这些漏洞百分比。 在最近的博客帖子&quot;[新的 Microsoft 漏洞报告揭示了 48% 的同比增加漏洞如何通过最低特权缓解这些漏洞&amp;](https://www.beyondtrust.com/blog/entry/microsoft-vulnerabilities-report)，90% 的严重漏洞在 Internet Explorer 中，85% 的严重漏洞在Microsoft Edge和 100% 的关键漏洞在 Microsoft Outlook 将通过删除管理员权限得到&quot;缓解。 为了支持安全帐户管理，组织需要确保已制定支持策略和过程，以促进安全最佳做法，并遵循这些策略和过程来缓解这些威胁。
...
<a id="control-40"></a>[控制 40](#control-40)：提供可证明的证据，证明在采样的系统组件中禁用、删除或更改默认凭据。
...
<a id="control-41"></a>[控制 41](#control-41)：提供可证明的证据，证明帐户创建、修改和删除通过已建立的审批过程。
...
<a id="control-42"></a>[控制 42](#control-42)：提供可证明的证据，证明存在禁用或删除 3 个月内未使用的帐户的流程。
...
- 示例证据准则：证据应为两倍。 首先，显示 &quot;作用域内环境中所有用户帐户的最后一个登录&quot; 名的屏幕截图或文件导出。 这可能是本地帐户以及集中目录服务中的帐户，例如 AAD (Azure Active Directory) 。 这将证明未启用超过 3 个月的帐户。 第二，季度审查过程的证据，可能是在 ADO (Azure DevOps) 或JIRA票证中完成任务的书面证据，或通过应注销的纸质记录。

- 示例证据：第一个屏幕截图显示了脚本的输出，该脚本每季度执行一次，以查看 AAD 中用户的最后一个登录属性。

![屏幕截图显示了脚本的输出，该脚本每季度执行一次，以查看 AAD 中用户的最后一个登录属性。](../media/UserGuidePhotos/Control-No-42.png)
...
<a id="control-43"></a>[控制 43](#control-43)：提供可证明的证据，证明已实施强密码策略或其他适当的缓解措施来保护用户凭据。  应将以下内容用作最低准则：
- 最小密码长度为 8 个字符
- 帐户锁定阈值不超过 10 次尝试
- 至少 5 个密码的密码历史记录
- 强制使用强密码
...
<a id="control-44"></a>[控制 44](#control-44)：提供可证明的证据，证明向所有用户颁发了唯一用户帐户。
...
<a id="control-45"></a>[控制 45](#control-45)：提供可证明的证据，证明环境中遵循的最小特权原则。
...
<a id="control-46"></a>[控制 46](#control-46)：提供可证明的证据，证明已制定过程来保护或强化服务帐户，并且正在遵循此过程。
...
<a id="control-47"></a>[控制 47](#control-47)：提供可证明的证据，证明为所有远程访问连接和所有非控制台管理接口配置了 MFA。
...
<a id="control-48"></a>[控制 48](#control-48)：提供可证明的证据，证明为所有远程访问连接和所有非控制台管理接口（包括访问任何代码存储库和云管理接口）配置了强加密。

定义为：
- **代码存储库** - 需要保护应用的代码库，防止恶意修改，因为恶意修改可能会将恶意软件引入应用。 需要在代码存储库上配置 MFA。
- **云管理接口** – 云服务提供商 (云解决方案提供商) 中托管了部分或全部环境，则此处包含云管理的管理接口。
...
<a id="control-49"></a>[控制 49](#control-49)：提供可证明的证据，证明 MFA 用于保护管理门户，用于管理和维护所有公共域名服务 (DNS) 记录。
...
<a id="control-50"></a>[控制 50](#control-50)：提供可证明的证据，证明入侵检测和预防系统 (IDPS) 部署在范围内环境的外围。
...
<a id="control-51"></a>[控制 51](#control-51)：提供可证明的证据，证明 IDPS 签名在) 的 24 小时内保持当前 (。
...
<a id="control-52"></a>[控制 52](#control-52)：提供可证明的证据，证明 IDPS 已配置为支持 TLS 检查所有传入的 Web 流量。
...
<a id="control-53"></a>[控制 53](#control-53)：提供可证明的证据，证明 IDPS 配置为监视所有入站流量流。
...
<a id="control-54"></a>[控制 54](#control-54)：提供可证明的证据，证明 IDPS 配置为监视所有出站流量流。
...
<a id="control-55"></a>[控制 55](#control-55)：为管理安全事件日志记录的最佳做法和过程提供策略文档。
...
<a id="control-56"></a>[控制 56](#control-56)：提供可证明的证据，表明在所有采样的系统组件中设置了安全事件日志记录以记录以下事件：
- 用户对系统组件和应用程序的访问权限
- 高特权用户执行的所有操作
- 逻辑访问尝试无效
- 特权帐户创建或修改
...
<a id="control-57"></a>[控制 57](#control-57)：提供可证明的证据，证明记录的安全事件包含以下最低信息：
- User
- 事件类型
- 日期和时间
- 成功或失败指标
...
<a id="control-58"></a>[控制 58](#control-58)：提供可证明的证据，证明所有采样的系统组件都按时间同步到同一主服务器和辅助服务器。
...
<a id="control-59"></a>[控制 59](#control-59)：在使用面向公众的系统时提供可证明的证据，即安全事件日志将发送到不在外围网络中的集中日志记录解决方案。
...
<a id="control-60"></a>[控制 60](#control-60)：提供明显的证据来证明集中日志记录解决方案受到保护，防止未经授权篡改日志记录数据。
...
<a id="control-61"></a>[控制 61](#control-61)：提供明显的证据，证明至少 30 天的安全事件日志记录数据立即可用，并保留 90 天的安全事件日志。
...
<a id="control-62"></a>[控制 62](#control-62)：提供管理日志评审做法和过程的策略文档。
...
<a id="control-63"></a>[控制 63](#control-63)：提供可证明的证据，证明日志由人工或自动化工具每天审查，以确定潜在的安全事件。

- 意向：此控件的目的是确保每天进行日志评审。这对于识别配置为提供安全事件警报的警报脚本/查询可能无法接收的任何异常非常重要。

- 示例证据准则：证据通常由屏幕截图或屏幕共享提供，表明正在进行日志评审。 这可能是通过每天完成的表单，或通过JIRA或DevOps票，并张贴相关评论，以显示这是每天执行的方式。 例如，可以创建&quot;每周JIRA票证，每日日志审阅 W/C 26 2021&quot; 年 6 月 26 日，每天有人发布每日日志评审结果。 如果标记了任何异常，则可以在此同一票证中记录此情况，以演示单个JIRA中的下一个控件。
...
<a id="control-64"></a>[控制 64](#control-64)：提供可证明的证据，证明调查和修正了潜在的安全事件和异常。

- 意向：目的是调查在每日日志审查过程中识别的任何异常，并执行适当的修正或操作。这通常涉及会审过程，以确定异常是否需要执行操作，然后可能需要调用事件响应过程。

- 示例证据准则：证据应通过屏幕截图提供，该屏幕截图演示标识为每日日志审阅一部分的异常。 如上所述，这可能是通过JIRA票证显示异常被标记，然后详细说明之后进行了哪些活动。 这可能会提示提高特定JIRA票证以跟踪正在执行的所有活动，或者可能只是记录在每日日志审阅票证中。 如果需要执行事件响应操作，则应将此记录为事件响应过程的一部分，并提供证据来证明这一点。
...
<a id="control-65"></a>[控制 65](#control-65)：提供管理安全事件警报做法和过程的策略文档。
...
<a id="control-66"></a>[控制 66](#control-66)：提供明显的证据，证明已针对以下类型的安全事件触发警报以进行即时会审：
- 特权帐户创建或修改
- 病毒或恶意软件事件
- 事件日志篡改
- IDPS 或 WAF 事件（如果已配置）
...
- 示例证据准则：证据应通过警报配置的屏幕截图 **和** 收到的警报的证据提供。 配置屏幕截图应显示触发警报的逻辑以及警报的发送方式。 警报可以通过短信、电子邮件、Teams通道、Slack 通道等发送...
...
<a id="control-67"></a>[控制 67](#control-67)：提供可证明的证据，表明工作人员始终全天可用来响应安全警报。
...
<a id="control-68"></a>[控制 68](#control-68)：提供证明已建立正式信息安全风险管理过程的证据。
...
<a id="control-69"></a>[控制 69](#control-69)：提供证明性的证据，证明每年至少进行一次正式的风险评估。
...
<a id="control-70"></a>[控制 70](#control-70)：提供可证明的信息安全风险评估包括威胁、漏洞或等效项的证据。
...
<a id="control-71"></a>[控制 71](#control-71)：提供可证明的信息安全风险评估包括影响、可能性风险矩阵或等效项的证据。
...
<a id="control-72"></a>[控制 72](#control-72)：提供可证明的信息安全风险评估包括风险寄存器和治疗计划的证据。
...
<a id="control-73"></a>[控制 73](#control-73)：提供 IRP)  (安全事件响应计划。
...
<a id="control-74"></a>[控制 74](#control-74)：提供明显的证据，证明安全 IRP 包括有案可查的通信流程，以确保及时通知主要利益干系人，如支付品牌和收购者、监管机构、监管机构、董事和客户。
...
<a id="control-75"></a>[控制 75](#control-75)：提供明显的证据，证明事件响应团队的所有成员都已完成年度训练或表顶级练习。
...
<a id="control-76"></a>[控制 76](#control-76)：提供可证明的证据，以显示根据经验教训或组织更改更新安全 IRP。
...
<a id="control-1b"></a>[控制 1](#control-1b)：提供证明 TLS 配置满足或超过 [TLS 配置](../docs/certification-submission-guide.md#appendix-a)要求中的加密要求的可证明证据。
...
<a id="control-2b"></a>[控制 2](#control-2b)：提供可证明的证据，证明 TLS 压缩在处理 Web 请求的所有面向公众的服务中处于禁用状态。
...
<a id="control-3b"></a>[控制 3](#control-3b)：提供可证明的证据，证明 TLS HTTP 严格传输安全已启用并配置为>= 15552000 跨所有站点。
...
<a id="control-4b"></a>[控制 4](#control-4b)：使用加密算法（例如 AES、Blowfish、TDES 和 128 位和 256 位的加密密钥大小）以内联方式加密静态数据的可证明证据。
...
<a id="control-5b"></a>[控制 5](#control-5b)：提供可证明的证据，证明哈希函数或消息身份验证 (HMAC-SHA1) 仅用于使用加密配置文件要求内联保护静态数据。
...
<a id="control-6b"></a>[控制 6](#control-6b)：提供所有存储数据的清单，包括存储位置和用于保护数据的加密。
...
<a id="control-7b"></a>[控制 7](#control-7b)：提供可证明的证据，证明已正式建立已批准和记录的数据保留期。
...
<a id="control-8b"></a>[控制 8](#control-8b)：提供可证明的证据，证明保留的数据与定义的保留期匹配。
...
<a id="control-9b"></a>[控制 9](#control-9b)：提供可证明的证据，证明进程已就绪，可在保留期后安全删除数据。
...
<a id="control-10b"></a>[控制 10](#control-10b):P对有权访问数据或加密密钥的所有个人的列表（包括业务理由）进行删除。
...
<a id="control-11b"></a>[控制 11](#control-11b)：提供可证明的证据，证明有权访问数据或加密密钥的采样人员已正式获得批准，并详细说明了其工作功能所需的权限。
...
<a id="control-12b"></a>[控制 12](#control-12b)：提供可证明的证据，证明有权访问数据或加密密钥的采样个人仅具有审批中包含的权限。
...
<a id="control-13"></a>[控制 13](#control-13b)：提供客户数据共享的所有第三方的列表。
...
<a id="control-14b"></a>[控制 14](#control-14b)：提供可证明的证据，证明所有使用客户数据的第三方都有共享协议。
...
<a id="control-15b"></a>[控制 15](#control-15b)： (SAR) 过程提供记录的主题访问请求，并提供证明数据主体能够提高 SAR 的证据。
...
<a id="control-16b"></a>[控制 16](#control-16b)：提供可证明的证据，证明你能够在响应 SAR 时识别数据主体数据的所有位置。
...
<a id="control-17b"></a>[控件 17](#control-17b)：提供指向隐私通知的链接，该链接应包含所有必需元素，如下所示：
