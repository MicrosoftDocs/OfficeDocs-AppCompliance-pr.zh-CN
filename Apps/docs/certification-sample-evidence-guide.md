...
- 控制 () ： = 评估活动说明 - 这些控制 () 和关联号码 (No.) 直接从 Microsoft 365 认证清单获得。  
...
- 意图：在你的环境中运行防病毒 (AV)  (或反恶意软件防护) ，以避免你可能或可能不知道的网络安全风险，因为潜在的破坏性攻击正在增加（在复杂程度和数量方面） 将 AV 部署到支持其使用的所有系统组件，将有助于降低环境中引入的反恶意软件的一些风险。 它仅需要取消保护单个终结点，以可能为威胁参与者提供攻击途径以进入环境。 因此，AV 应用作多个防护层之一，以抵御这种类型的威胁。
...
- 意图：每天识别 PUA (数十万) 和可能不需要的应用程序。 为了针对新发布的恶意软件提供充分保护，需要定期更新 AV 签名，以考虑新发布的恶意软件。
...
**注意：** 如果未启用访问扫描，则至少必须启用每日扫描 **alerting__be** 扫描。

- 意图：此控件的目的是确保快速识别恶意软件，以最大限度地减少这对环境的影响。 执行访问扫描并结合自动阻止恶意软件时，这有助于阻止防病毒软件已知的恶意软件感染。 如果由于误报&#39;服务中断而不需要访问扫描，则适合的 (或) 或更多的) 扫描和警报机制需要实施，以确保及时响应恶意软件感染，以最大限度地减少损害。

- 示例证据指南：为样本中支持防病毒的每台设备提供屏幕截图，显示防病毒正在设备上运行，并配置为用于访问 (实时扫描) 扫描，或者提供一个屏幕截图，显示为每日扫描启用了定期扫描、为示例中的每台设备配置了警报和上次扫描日期。 
...
- 示例证据指南：证据可能通过以下方法提供：显示邮件列表订阅，手动查看新发布的漏洞的安全源 (将需要使用活动的时间戳进行充分跟踪，例如，使用 JIRA 或 Azure DevOps) ，查找过期软件库时 (工具可能是 Snyk， 或者可能是使用经过身份验证的扫描识别过时 software.) 。
...
以下控件适用于修补程序管理的修补元素。 若要维护安全的操作环境，必须适当修补应用程序/加载项和支持系统。 需要管理标识 (公开发布版本) 以及修补之间的适当时间范围，以减少威胁参与者利用漏洞的机会窗口。 该Microsoft 365认证未规定&#39;修补窗口&#39;，但是认证分析师将拒绝不合理的时间范围。
...
- 示例证据指南：提供示例中每个设备的屏幕截图，其中显示了运行 (的操作系统版本，&#39;屏幕截图中) 。 除此之外，还提供在环境中运行的软件组件正在运行受支持版本的证据。 这可以通过提供内部漏洞扫描报告输出 (提供经过身份验证的扫描包含在) 和/或检查第三方库的工具输出（如[Snyk、Trivy](https://snyk.io/)或[NPM 审核](https://docs.npmjs.com/cli/v7/commands/npm-audit)）[](https://github.com/aquasecurity/trivy)中完成。 如果仅在 PaaS 中运行，修补控制组仅需要涵盖第三方库修补。
...
此屏幕截图显示作用域内系统组件 &quot; CLARANET-SBU-WM 在受支持的 Windows &quot; 版本上运行。
...
- &quot;[名为 PCI DSS 合规性的 2020](https://info.securitymetrics.com/pci-guide-2020)年安全指标指南的安全指标报告指出，从发现组织存在漏洞使攻击者破坏系统起，&#39;平均需要 &quot; 166 天。 一旦泄露，攻击者平均可以访问敏感数据 127 天&#39;因此此控制旨在识别范围内环境中的潜在安全漏洞。

- 示例证据指南：提供 (12) 季度&#39;漏洞扫描的完整扫描报告。 报告应清楚地说明用于验证是否包含完整公共占用以及每个内部子网（如果适用）的目标。 提供 **每个季度** 的所有 **扫描** 报告。
...
- 示例证据：下面是我们需要的防火墙策略文档类型的示例 (这是一个演示，可能不会) 。
...
- 意图：组织需要注意供应商提供的默认管理凭据，这些凭据在设备或软件配置过程中配置。 默认凭据通常由供应商公开提供，并且可为外部威胁参与者提供一个破坏环境的机会。 例如，在 Internet 上简单搜索默认 iDrac (集成 Dell 远程访问控制器) 凭据将突出显示 _root：：calvin_ 作为默认用户名和密码。 这将允许某人远程访问远程服务器管理。 此控件的目的是确保环境不易受在设备/应用程序强化期间未更改的默认供应商凭据的攻击。
...
- 意图：防火墙提供控制不同安全级别的不同网络区域之间的流量的能力。 由于所有环境都连接了 Internet，防火墙需要安装在边界上，即 Internet 和作用域内环境之间。 此外，需要在信任度较低 DMZ (非) 网络与内部受信任网络之间安装防火墙。 DMZ 通常用于为来自 Internet 的流量提供服务，因此是攻击目标。 通过实施 DMZ 和使用防火墙控制流量，DMZ 泄露&#39;不一定意味着内部受信任网络和公司/客户数据受到威胁。 应提供足够的日志记录和警报来帮助组织快速识别泄露，以最大限度地降低威胁参与者进一步破坏内部受信任网络的机会。 此控件的目的是确保受信任网络与受信任度低的网络之间有足够的控制。

- 示例证据指南：应该通过防火墙配置文件或屏幕截图提供证据，以证明 DMZ 已就位。 这应该与提供的体系结构图表匹配，这些图表演示了支持环境的不同网络。 防火墙上的网络接口的屏幕截图，与作为初始文档提交一部分提供的网络图一起应提供此证据。

- 示例证据：下面是演示两个 DMZ 的 WatchGuard 防火墙的屏幕截图，一个适用于名为 DMZ) 的入站服务 (，另一个为 Bastian Host) 提供 jump (box 服务。

![图片](../media/UserGuidePhotos/Control-No-20.png)

**控件 No. 21：** 提供可证明所有公共访问终止于 DMZ 安全 (的可) 。
...
- 示例证据指南：可通过防火墙配置提供证据，或者通过屏幕截图提供证据，这些屏幕截图显示在末尾显示"全部拒绝"规则的所有防火墙规则，或者如果防火墙丢弃了默认不与规则&#39;匹配的流量，则提供所有防火墙规则的屏幕截图和指向供应商管理指南的链接，突出显示默认情况下防火墙将丢弃所有不匹配的流量。 &quot; &quot;
...
- 示例证据指南：如果配置提供非控制台管理接口的加密配置，则提供防火墙配置 (并非所有设备都将此配置作为可配置选项) 。 如果此&#39;不在配置内，你或许能够向设备发出命令，以显示为这些连接配置了哪些内容。 某些供应商可能在文章中发布此信息，因此这也可能成为证明此信息的一种方法。 最后，您可能需要运行工具以输出支持哪些加密。
...
- 示例证据指南：证据需要能够证明审查会议已经发生。 这可以通过共享防火墙审查的会议分钟数和任何其他更改控制证据（显示从审查中执行的任何操作）来完成。 确保日期存在，因为&#39;至少需要查看其中两个会议 (，即每六个月查看一) 
...
可以选择将 Web 应用程序防火墙 (WAF) 解决方案中。 如果使用 WAF，这将计为安全域中"操作安全"&#39;评分&#39;额外评分。 WAF 可以检查 Web 流量，以筛选和监视 Internet 和已发布 Web 应用程序之间的 Web 流量，以确定特定于 Web 应用程序的攻击。 Web 应用程序可能会遭受许多特定于 Web 应用程序的攻击，如 SQL 注入 (SQLi) 、跨站点脚本 (XSS) 、跨站点请求伪造 (CSRF/XSRF) 等。WAF 旨在防范这些类型的恶意负载，以帮助保护 Web 应用程序免受攻击和潜在威胁。

**控件 No. 26：** 提供充分证据，表明 WAF (防火墙) 配置为主动监视、警报和阻止恶意流量。

- 意图：此控件用于确认 WAF 已针对所有传入 Web 连接就位，并且已配置为阻止恶意流量或向恶意流量发出警报。 若要为 Web 流量提供额外的防护层，需要针对所有传入的 Web 连接配置 WAF，否则外部威胁参与者可以绕过旨在提供此额外保护层的 WAF。 如果未将 WAF 配置为&#39;主动阻止恶意流量，则 WAF 需要能够立即向可快速响应潜在恶意流量的员工发出警报，以帮助维护环境的安全性并停止攻击。

- 示例证据指南：提供来自 WAF 的配置输出，该输出突出显示了要提供的传入 Web 连接，并且该配置会主动阻止恶意流量或正在监视和警报。 或者，可以共享特定设置的屏幕截图，以演示组织是否满足此控制要求。

- 示例证据：以下屏幕截图显示了 Contoso 生产 Azure 应用程序网关 WAF 策略已启用，并且配置为 &#39;Prevention&#39; 模式，该模式将主动丢弃恶意流量。
...
以下屏幕截图显示了 Contoso \_ AppGW \_ CoreRules，它适用于 api.contoso.com 服务。
...
**注意：** 可以在开发/测试环境中使用实时数据，提供开发/测试包含在评估范围内，以便根据认证控制措施对安全性Microsoft 365评估。

- 示例证据指南：可通过共享针对生产数据库 SQL 查询输出的屏幕截图来提供证据 (修订任何敏感信息) 以及开发/测试数据库。 相同命令的输出应生成不同的数据集。 在存储文件的位置，查看两个环境中的文件夹的内容还应演示不同的数据集。

- 示例证据：以下屏幕截图显示了用于 (前 3 条记录，请提供来自生产) 前 20 个记录。
...
参与软件开发活动的组织经常面临安全和 TTM (上市时间) 压力之间的竞争优先级，但是，在整个软件开发生命周期中实施与安全相关的活动 (SDLC) 不仅可以节省资金，还可以节省时间。 将安全性作为后向安全处理时，通常仅在 () 的测试阶段识别问题，这通常更耗时且修复成本也高。 此安全部分的目的是确保遵循安全软件开发实践，以减少在开发的软件中引入编码缺陷的风险。 此外，本节还介绍一些控件，帮助安全部署软件。
...
- 示例证据指南：提供代码经过对等审查且必须经过授权才能应用于生产环境的证据。 此证据可能是通过导出更改票证，证明已执行代码评审并授权更改，或者通过代码审阅软件（如 Crucible ([https://www.atlassian.com/software/crucible](https://www.atlassian.com/software/crucible)) ）。

- 示例证据

![图片](../media/UserGuidePhotos/Control-No-35.png)下面是一个票证，显示代码更改由原始开发人员外的其他人员进行审阅和授权过程。 它显示代码评审已由被分配者请求，并将分配给其他人进行代码审阅。

下图显示了代码审阅已分配给原始开发人员外的其他人员，如下图右侧突出显示的部分所示。 在左侧，可以看到代码已经过审阅，并且&#39;代码审阅&#39;状态。
...
- 意图：如果威胁参与者可以访问和修改软件&#39;基本代码，他/她可能会向代码库（从而向应用程序）引入漏洞、后门或恶意代码。 这已经有很多实例，其中最公开的可能是 NotPetya 勒索软件攻击，该攻击报告通过名为 M.E.Doc (的乌克兰税务软件更新受到感染，请参阅 What [is NotPetya](https://www.itpro.co.uk/malware/34381/what-is-notpetya)) 。
...
安全帐户管理做法非常重要，因为用户帐户是允许访问信息系统、系统环境和数据的基础。 由于用户&#39;凭据泄露，用户帐户需要正确保护，不仅可以提供进入环境的页脚和访问敏感数据的权限，还可以提供对整个环境或关键系统的管理控制（如果用户&#39;凭据具有管理权限）。
...
-  BeyondTrust 每年会生成一份 Microsoft 漏洞报告，该报告分析上一年的 Microsoft 安全漏洞，并详细介绍依赖于具有管理员权限的用户帐户的这些漏洞 &quot; &quot; 的百分比。 最近博客文章 New &quot; [Microsoft Vulnerabilities Report Reveals a 48%YoY Increase in Vulnerabilities How could Be &amp; Mitigated with Least Privilege，](https://www.beyondtrust.com/blog/entry/microsoft-vulnerabilities-report) &quot; 90% of Critical vulnerabilities in Internet Explorer， 85% of Critical vulnerabilities in Microsoft Edge and 100% of Critical vulnerabilities in Microsoft Outlook would be been mitigated by removing admin rights. 为了支持安全帐户管理，组织需要确保制定支持性策略和程序，以提升安全性最佳做法，并遵循这些策略和过程来缓解这些威胁。
...
下一张屏幕截图显示了在作用域内系统组件 &quot; CLARANET-SBU-WM 上禁用来宾帐户 &quot; 。
...
- 示例证据指南：证据应为双重证据。 首先，显示范围内环境中所有用户帐户的上次登录的屏幕截图 &quot; &quot; 或文件导出。 这可能是本地帐户以及集中目录服务中的帐户，例如AAD (Azure Active Directory) 。 这将演示未启用任何超过 3 个月的帐户。 其次，季度审阅流程的证据，可能是 ADO (Azure DevOps) 或 JIRA 票证中完成的任务的书面证据，或者通过应注销的书面记录。
...
- 意图：如前面所讨论，用户凭据通常是威胁参与者试图在环境中获取对组织的访问权限&#39;攻击的目标。 强密码策略的目的是尝试并强制用户选取强密码，以减少威胁参与者能够暴力攻击他们的可能性。 添加 或其他合适的缓解措施的目的是认识到，组织可能会实施其他安全措施来帮助保护基于行业开发（如 &quot; &quot; &quot; [NIST 特殊出版物 800-63B）](https://pages.nist.gov/800-63-3/sp800-63b.html)的用户凭据 &quot; 。
...
- 意图：只应为用户提供满足其工作职能所需的权限。 这是为了限制用户有意或无意访问不应访问&#39;或执行恶意操作的风险。通过遵循此原则，它还可以减少潜在的攻击面 (即特权帐户) 恶意威胁参与者的目标。
...
以下屏幕截图显示"&#39;帐户是敏感帐户，&#39;服务帐户"上选择了"SQL &quot; \_ 委派连接"选项 &quot; 。

![图片](../media/UserGuidePhotos/Control-No-46.png)

下一张屏幕截图显示服务帐户 Prod SQL 服务帐户锁定到 SQL Server &quot; \_ &quot; 并且只能登录到该服务器。
...
- 意图：此控件的目的是提供缓解措施，防止强制特权帐户和具有安全访问权限的帐户进入环境。 通过向 MFA (多重) ，应仍然防止泄露的密码成功登录，因为 MFA 机制仍应受到保护。 这有助于确保所有访问和管理操作仅由授权和受信任的员工执行。
...
此屏幕截图演示启用了指向 Duo-LDAP 的其他身份验证服务器，该服务器针对 &#39;&quot; &quot; - 默认路由&#39;身份验证领域。
...
- 示例证据指南：提供证明公共 DNS 管理门户受 MFA 保护的证据。 即使公共 DNS 承载在作用域内环境（即由组织) 控制和运行）内的服务器上 (也可能有一个管理门户注册域名，DNS 记录是 &#39;托管的&#39;，用于将 DNS 服务器指向您自己的基础结构。 在这种情况下，如果可以修改域 DNS 记录，应在域注册机构管理界面上启用 MFA。 应提供屏幕截图，显示系统级别为 MFA 启用了管理界面 (即所有特权帐户) 。

- 示例证据：以下屏幕截图显示 contoso corporation contoso.com DNS Microsoft Azure管理。
...
- 示例证据：尽管此屏幕截图&#39;24 小时内 IDPS 签名已更新，但它确实演示已安装最新版本，该版本是一周前在 _18__th_ ) 上收集的 (证据。 这一点与下面的屏幕截图相结合，显示签名将在 24 小时内是最新的。
...
- 示例证据 4：Azure 最近开始提供名为"Azure 防火墙 高级版"的 IDPS，这将允许通过策略配置 TLS、威胁智能、IDPS，但请注意，由于 Azure 防火墙 高级版 不支持入站 SSL 连接上的 IDPS，因此你仍然需要使用前端网关或应用程序网关对入站流量进行 SSL 卸载。
...
- 示例证据 1：以下屏幕截图显示了名为 &quot; "一个被采样的设备（名为"VICTIM1-WINDOWS"）的配置设置 &quot; 。 这些设置显示在"本地安全策略"&#39;"本地策略"审核策略设置&#39;设置。
...
- 示例证据：以下屏幕截图显示了来自作用域内系统组件 &quot; SEGSVR02 Windows事件查看器中安全事件的信息 &quot; 。
...
- 示例证据：Contoso DMZ 系统利用 NXLog 来寄送日志文件。 以下屏幕截图显示了&#39;所有 DMZ&#39; &quot; DESKTOP-7S65PN DMZ 跳转框上运行的 nxlog 服务 &quot; 。
...
以下 NXLog () 通过以下提取显示日志 [https://nxlog.co/documentation/nxlog-user-guide/modes.html](https://nxlog.co/documentation/nxlog-user-guide/modes.html) 交付：
...
注意：由于这是一个面向公众的文档，防火墙序列号已被修订，但我们&#39;ISV 来支持任何修订的屏幕截图，除非它包含个人身份信息。
...
最后一张屏幕截图标识密码策略更改在设置策略时发生的位置，因此帐户密码&#39;不会过期。
...
- 示例证据：以下屏幕截图示例显示由 Claranet 网络安全 MDR 在 ServiceNow 中跟踪的安全警报 (Managed Detection and Response) SOC。
...
信息安全风险管理是一项重要活动，所有组织都应至少每年执行一次。 组织必须了解其威胁和风险才能有效缓解这些威胁。 如果没有有效的风险管理，组织可能会实施他们认为非常重要的领域的安全最佳做法，因此，如果其他威胁的可能性很大，因此应该缓解这些威胁，则这些组织可能会在这些方面投入资源、时间和资金。 有效的风险管理可帮助组织重点关注对业务构成最大威胁的风险。 由于安全环境不断变化，因此威胁和风险可能会更改加班时间，因此应每年执行一次。 在 COVID-19 中可以发现一个很好的示例，它看到了大量钓鱼攻击和大规模 (和快速) 推出远程工作（数百或数千名工作人员）。
...
- 示例证据指南：证据应该通过风险注册/风险处理计划) （其中应包括风险和漏洞）提供，不仅应包含已提供的信息安全风险评估过程，还应提供风险评估 (的输出。
...
- 示例证据指南：不仅应该通过已提供的信息安全风险评估流程提供证据，还应通过风险注册/风险处理计划 (提供证据) 其中应包括影响和可能性评级。

- 示例证据：以下屏幕截图显示了风险注册，其中演示了影响和可能性。

![图片](../media/UserGuidePhotos/Control-No-71.png)

**注意：** 应提供assessment_ _document__ation风险，而不是屏幕截图。
...
- 意图：组织可能根据它们运营的国家/地区（例如，一般数据保护 (国家/地区）履行泄露通知义务;GDPR) ，或根据所提供的功能（例如，PCI DSS (支付数据时提供的功能) 。 因此，如果未能及时通知，则可能会造成严重后果，以确保满足通知义务，事件响应计划应包括一个通信过程，其中包括与所有利益干系人的通信、媒体通信过程以及谁可以与媒体通信，哪些人不能与媒体通信。
...
- 意图：随着时间的推移，IRP 事件 (计划) 组织更改或基于在购买 IRP 时学到的经验而改进。 对操作环境的更改可能需要更改 IRP，因为威胁可能会更改，或者法规要求可能会更改。 此外，随着表表练习和实际安全事件响应的执行，这通常可标识 IRP 中可以改进的区域。 这需要内置到计划中，并且此控件的目的是确保此过程包含在 IRP 中。
...
包含此安全域是为了确保 M365 使用的任何数据在传输和其余过程中都受到充分保护。 根据与欧盟公民隐私) 一般数据保护条例 (GDPR) ，此域还可确保消费者 (数据主体) ISV 正在解决隐私问题。
...
- 示例证据指南：证明这一点的最简单方法是针对所有 **Web** 侦听器（包括在非标准端口上运行的任何侦听器）运行 [Qualys SSL 服务器](https://www.ssllabs.com/ssltest/)测试工具。

- 请记住，选中"不在板上显示结果"选项，这将阻止 &quot; &quot; 将 URL 添加到网站。

- 您还可以提供证据，以演示 TLS 配置文件配置要求中的个人检查。 配置设置可以与脚本和软件工具一起使用，以帮助提供某些特定设置的证据，即 TLS 压缩已禁用。

- 示例证据：以下屏幕截图显示了 web 侦听器 [www.clara.net:443](http://www.clara.net:443/) 的结果。
...
- 意图：存在特定的 TLS 漏洞，即 ([CVE-2012-4929](https://nvd.nist.gov/vuln/detail/CVE-2012-4929)) ，它会影响 TLS 压缩。 因此，行业建议关闭此功能。
...
**控件 No. 3：** 提供可证明 TLS HTTP 严格传输安全性已启用且配置为在所有>为 15552000 的明显证据。

- 意图：HTTP 严格传输安全 (HSTS) 是一种安全机制，旨在通过强制 TLS 连接通过名为 &quot; Strict-Transport-Security 的 HTTPS 响应头字段来保护网站免受中间人攻击 &quot; 。

- 示例证据指南：这可以通过 Qualys SSL 实验室工具或其他工具和 Web 浏览器外接程序提供证据。

- 示例证据：以下屏幕截图通过名为 &#39;HTTP Header Spy&#39; 的 Web 浏览器外接程序 www.microsoft.com [这一](http://www.microsoft.com/) 点。
...
- 示例证据 2：以下屏幕截图Azure 存储对 blob 和文件进行加密配置。 以下屏幕截图显示了 Microsoft 文档页Azure 存储静态数据的加密，其中显示Azure 存储 &quot; [](https://docs.microsoft.com/en-gb/azure/storage/common/storage-service-encryption) &quot; AES-256 进行加密。
...
- 意图：若要正确保护数据，组织需要了解其环境/系统正在使用哪些数据以及数据存储的位置。 完全理解并记录这一点后，组织不仅能够实施足够的数据保护，还可以整合数据所在的位置，以便更有效地实施保护。 此外，在将数据合并到尽可能少的位置时，实现足够的 RBAC (基于角色的访问控制) 以将访问权限限制为尽可能少的员工会容易得多。

- 示例证据指南：证据应该通过文档提供或从内部系统（即 SharePoint 或 Confluence）导出，其中详细说明使用的所有数据、所有存储位置以及实现的加密级别。
...
当 ISV 使用和存储 M365 数据时，如果威胁参与者破坏 ISV 环境，这将会面临数据泄露的风险。 为了最大程度地降低这种风险，组织应仅保留他们需要的交付服务数据，而不是将来 &quot; &quot; 可能使用的数据。 此外，只有在需要提供已捕获数据的服务时，才应保留数据。 应定义数据保留并与用户进行通信。 数据超过定义的保留期后，必须安全删除该数据，以便无法重新构造或恢复数据。
...
- 示例证据指南：提供完整的数据保留策略，该策略清楚地详细说明了数据 (必须涵盖) 数据类型，以便业务可以执行业务功能。
...
- 示例证据指南：提供屏幕截图证据 (或通过屏幕共享) 显示所有不同数据位置（例如数据库、文件共享、存档等）中存储的数据 () 不超过定义的数据保留策略&#39;。 例如，具有日期字段的数据库记录的屏幕截图、按最旧的记录顺序搜索的屏幕截图和/或显示保留期内时间戳的文件存储位置。

**注意：** 应在屏幕截图中修订任何个人/敏感的客户数据。

- 示例证据：以下证据显示一个 SQL 查询，该查询显示按升序排序的数据库表的内容，并按升序在 &#39;DATE TRANSACTION&#39; 字段中显示数据库中最早的 \_ 记录。 此数据应长于两个月，&#39;超过定义的保留期。
...
- 示例证据指南：通常，为上一个控件提供的证据可帮助支持此控制。 如果没有对&#39;文档进行正式审批，则证据可能包含在工具（如 Azure DevOps 或 Jira）中针对访问而引发和批准的更改请求。
...
大多数组织将处理可能是欧洲公民&#39;的数据 (数据主体) 数据。 处理 **任何数据** 主体的数据时，组织需要符合 GDPR 一般数据保护 (规定) 。 这适用于直接捕获 (数据控制者) 或数据处理者 (你代表数据控制者处理此数据) 。 虽然此部分&#39;整个法规，但它涉及 GDPR 的一些关键要素，以帮助获得组织认真对待 GDPR 的某种保证。

**控件 No. 15：** 在 SAR 过程中提供记录 (访问) 并提供证据，证明数据主体能够提出 SAR。

- 意图：GDPR 包括处理数据主体或数据处理组织&#39;义务。 组织管理主体访问请求 (SAR) 的义务包含在第 12 条中，该条款根据第 12.3 条向数据控制者提供收到 SAR 的一个月以响应请求。 如有必要，允许再延长两个月。 即使你的组织充当数据处理者，也仍然需要这样做来帮助客户履行 (控制者) 履行 SAR 义务。
...
- 意图：此控件的目的是确保组织具有一个可靠的机制，用于标识数据&#39;主体。 这可能是一个手动过程，因为所有数据存储都记录良好，或者可能使用其他工具确保所有数据都作为 SAR 过程的一部分进行定位。
...
下面的四个图像显示了 ISV 数据位置（查询位置和 存储 资源管理器）如何使用来深入了解需要从存储中删除以符合 SAR 请求的文件或 blob。
...
Azure 的上图显示了如何配置 Azure 以满足存储在后端环境中的数据的 GDPR 合规性要求。 策略 (（可从 _Azure 蓝图_) 自定义或生成）允许 ISV 确保客户端&#39;数据正确存储，并且仅通过设置的度量和警报进行访问，以确保合规性，并且将在合规性管理器仪表板上显示不兼容的数据或用户访问。
...
- "行动欺诈网络犯罪报告"，位于 [https://www.actionfraud.police.uk/](https://www.actionfraud.police.uk/) ： (2021 年 8 月 21 日) 。
- 欧盟。  (2021) 数据控制者 GDPR 清单，位于 [https://gdpr.eu/checklist/](https://gdpr.eu/checklist/) ： (2021 年 1 月 21 日访问) 。
- Microsoft.  (2018) 事件日志记录 (Windows Installer) 提供时间：docs.microsoft.com/en-us/windows/win32/msi/event-logging (Accessed： 23/12/20) 。
- 正技术。  (2020) 如何进行安全软件开发：可用时间 [https://www.ptsecurity.com/ww-en/analytics/knowledge-base/how-to-approach-secure-software-development/](https://www.ptsecurity.com/ww-en/analytics/knowledge-base/how-to-approach-secure-software-development/) ： (Accessed：21/12/20) 。
- 有关 (个人数据处理和此类数据的自由移动保护的欧盟第 2016/2016/679 号欧盟) 2016/679 条例和 2016 年 4 月 27 日《关于保护自然人和此类数据自由移动的法规》，并批准第 95/46/EC (号一般数据保护条例)  (Text with EEA relevance)  (2016) ，位于： [https://www.legislation.gov.uk/eur/2016/679/contents](https://www.legislation.gov.uk/eur/2016/679/contents) (访问时间：2021 年 11 月 1) 。
...
- Qualys。  (2014) SSL 实验室：信任 (T) 新成绩和不匹配 (M) 问题，地址 [https://blog.qualys.com/product-tech/2014/06/17/ssl-labs-new-grades-for-trust-t-and-mismatch-m-issues](https://blog.qualys.com/product-tech/2014/06/17/ssl-labs-new-grades-for-trust-t-and-mismatch-m-issues) ： (Accessed：29/01/21) 。
- NIST SP800-61r2：计算机安全事件处理指南，可从 [https://csrc.nist.gov/publications/detail/sp/800-61/rev-2/final](https://csrc.nist.gov/publications/detail/sp/800-61/rev-2/final) ： (24/01/21 访问) 。

**从 Microsoft 文档拍摄的图像**

- [https://www.sans.org/information-security-policy/](https://www.sans.org/information-security-policy/) (2021 年 18 月 21 日) 。
- [https://docs.microsoft.com/en-us/cloud-app-security/anomaly-detection-policy](https://docs.microsoft.com/en-us/cloud-app-security/anomaly-detection-policy) (2011 年 16 月 21 日) 。
