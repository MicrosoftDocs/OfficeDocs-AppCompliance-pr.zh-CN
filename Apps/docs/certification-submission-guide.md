...
> [!NOTE]
> 你无需遵守本认证规范Microsoft 365认证规范的所有控制措施。 但是，对于本 (规范中讨论的每个安全域) Microsoft 365不会披露的阈值。 某些控件将被分类为"硬失败"，这意味着缺少这些安全控制将导致评估失败。 
...
其中 IaaS 和/或 PaaS 用于支持审核中的应用程序或外接程序代码交付的基础结构，云平台提供商将负责在整个认证过程中评估的一些安全控制。 因此，云平台提供商需要通过外部合规性报告（如 [PCI DSS](bookmark://pci-dss)合规性证明   (AOC) 、ISO27001 或 [SOC 2](bookmark://soc-2)类型 II 报告）向认证分析师提供独立的外部安全最佳做法验证。   
...
执行 GraphAPI 权限验证以验证应用/外接程序不会请求过度许可的权限。 这是通过手动检查请求的权限来实施。 认证分析师将针对 Publisher 提交交叉引用这些检查，并评估请求的访问级别以确保满足"最小特权"做法。 如果认证分析师认为不满足这些"最小特权"做法，认证分析师将就验证所请求权限的业务理由进行开放式讨论。 在此审查过程中发现Publisher证明提交的任何差异也将获得反馈，以便Publisher证明。 
...
|**WAF (Web 应用程序防火墙)  (可选) ：** 对于满足以下控件，将支付额外的信用。|提供充分证据，表明 WAF (防火墙) 配置为主动监视、警报和阻止恶意流量。|
...
|| 提供可证明代码存储库通过 MFA 身份验证和 MFA (保护的) 。|
...
|**GDPR** (如果适用) | 提供记录的主题访问请求 (SAR) ，并提供证据，证明数据主体能够提出 SAR。|
...
| **标准** | **要求** |
| ----- | ----- |
| **[ISO 27001](#iso-27001)** | 需要面向公众的 ISO  () 声明和 ISO 27001 证书副本。  该 ISO 总结了你在 114 个信息安全控制措施中的位置，并用于确定 ISO 27001 证书中是否排除任何不令人信意的控制措施。 如果无法通过查看面向公众的 ISO 版本来确定这一点，那么，如果 ISO 27001 将用于验证某些 Microsoft 365 认证规范控件，则分析员可能需要访问完整的 ISO。  除了验证 ISO 27001 评估活动的范围之外，分析师还将确认审核公司的有效性，如上所述。|
...
如果外部安全框架已包含在 Publisher 证明中，则认证分析人员将需要在认证评估中检查这些安全合规性Microsoft 365的有效性。
...
> [!NOTE]
> 尽管上述外部安全标准/框架可以提交为证据，以满足某些 Microsoft 365 认证控制措施，但通过 Microsoft 365 认证并不意味着你将成功通过针对这些标准/框架的审核。 《Microsoft 365 认证规范》只是这些安全标准/框架的一小部分，它允许 Microsoft 在安全状态方面获得一级保证。
...
&#x2713;支持的外部安全与合规框架必须是最新的，即过去 12 个月内 (或 15months 内（如果当前正在进行重新评估，并且可在) 中提供证据）。
...
如果已实现 ISO27001 合规性，则以下增量的 (差距) ISO 27001 未完全覆盖，至少需要作为本 Microsoft 365 认证的一部分进行审查。

> [!NOTE]
> 作为 Microsoft 365 认证评估的一部分，认证分析师将确定任何映射的 ISO 27001 控制措施是否未作为 ISO 27001 评估的一部分包含在内，并可能决定对发现包含的控制措施进行采样以提供进一步保证。 ISO 27001 中缺少的任何要求都将包含在你的认证Microsoft 365活动中。
...
*   演示通过防火墙保护 (的所有) 都经过授权过程，这导致记录具有业务理由的所有流量。
...
&#x2713;与 NIST 网络安全框架保持一致的事件处理 (识别、保护、检测、响应、恢复) 。
...
*   提供或演示支持文档，详细说明了如何配置应用程序控制软件以满足特定应用程序控制机制 (例如允许列表、代码签名等) 。
...
由于 PCI DSS 审核未专门评估 IDPS (IDPS) 的一些元素，因此这将要求您：
...
如果已获得 SOC 2 合规性，则以下增量的 (差距) SOC 2 未完全涵盖的漏洞需要作为本 Microsoft 365 认证一部分进行审核。
...
* 演示所有允许通过防火墙 () 流量都经过授权过程，授权过程会记录所有流量以及业务理由。
...
由于 SOC 2 审核未专门评估 IDPS (的入侵检测和防护服务) ，因此需要您：
...
|**混合托管**|使用混合托管类型，可以利用多种托管类型来支持支持环境的各个部分。 这可能是在多个 M365 堆栈中利用应用/加载项的情况。 尽管 Microsoft 365 认证将支持跨多个 M365 服务开发应用/加载项的地方，但需要根据每个适用的"托管类型映射"评估跨应用/外接程序) 支持环境的整个 (。 有时，您可以对单个外接程序使用不同的托管类型，执行此操作时，条件的适用性仍然需要在各种托管类型中遵循"托管类型映射"条件。|
|**共享托管**|共享托管是在由多个单个使用者共享的平台中托管环境的地方。 由于Microsoft 365云，共享托管并不常见，因此未写入"Microsoft 365认证规范"来说明这一点。 如果认为正在使用此功能，请联系 Microsoft，因为需要创建其他要求，以考虑此类型托管类型下的其他风险。|
